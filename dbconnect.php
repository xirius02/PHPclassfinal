...
function wholebaseprojects()
{
    
    $db = getDatabase();
            $stmt = $db->prepare("SELECT * FROM newproject");
        /*
         * We execute the statement and make sure we
         * got some results back.
         */
...
function checkin($id)
{
    ...
    $db = getDatabase();
    //$stmt = $db->prepare("update newproject set checkin = now(), checkstat = 1 where id = '$id'");
    $stmt = $db->query("update newproject set checkin = now(), checkstat = 1 where id = '$id'");
...
function checkout($id)
{
    ...
    $db = getDatabase();
    //$stmt = $db->prepare("update newproject set checkin = now(), checkstat = 1 where id = '$id'");
    $stmt = $db->query("update newproject set checkout = now(), checkstat = 0 where id = '$id'");
